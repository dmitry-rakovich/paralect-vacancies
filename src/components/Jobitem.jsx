...
export default function Jobitem({vacancy: {payment_from, payment_to, profession, currency, type_of_work, town, id}, getVacancies}) {
  const [isFavorite, setIsFavorite] = useState(JSON.parse(localStorage.getItem('favorites'))?.includes(id) || false)

  function handleFavorite() {
    const favorites = JSON.parse(localStorage.getItem('favorites')) || []
    if(isFavorite) {
      const newFavorites = JSON.stringify([...favorites.filter(item => item !== id)])
  ...
  return (
    ...
    <div data-elem={`vacancy-${id}`} className="vacancy_item">
      ...
      <svg data-elem={`vacancy-${id}-shortlist-button`} onClick={handleFavorite} className={isFavorite ? 'favorite_button select': 'favorite_button'} width="24" height="24" viewBox="0 0 24 24" fill="" xmlns="http://www.w3.org/2000/svg">
